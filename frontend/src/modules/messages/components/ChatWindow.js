 ...
 * ChatWindow
 * Props:
 *   conversation   — { conversationId, subject, allowReply, participants[] }
 *   currentUserId  — string
 *   onArchive()
 *   onBack()       — mobile back button
 */
export default function ChatWindow({ conversation, currentUserId, onArchive, onBack }) {
  const { messages, loading, error, markRead, sendReply, refetch } =
    ...
    <>
      ...
      <div className="thread-header">
        ...
        <div className="thread-actions">
          ...
          <button className="icon-btn" title="Archive conversation" onClick={onArchive}>
      ...
      <div className="thread-messages">
        ...
        {error && (
          <div style={{ color: 'var(--danger)', padding: 16, fontSize: 13 }}>
            Failed to load messages. <button onClick={refetch} style={{ color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer' }}>Retry</button>
          </div>
        )}
