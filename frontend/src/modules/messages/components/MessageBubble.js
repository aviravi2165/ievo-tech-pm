 ...
 * MessageBubble.js
 *
 * FIX: handleDownload() now calls fileApi.download(attachmentId, originalName)
 * instead of the old getDownloadUrl() + anchor-click trick.
 *
 * The old approach:
 *   1. Built a plain URL (no Authorization header).
 *   2. Opened it in a new tab → triggered a 401 on JWT-protected routes.
 *
 * The new approach uses fetch() with Bearer token → converts to Blob →
 * triggers Save-As dialog.  See fileApi.js for implementation details.
 */
 ...
 * MessageBubble
 * Props:
 *   message  — { messageId, senderName, bodyHtml, sentAt, attachments[],
 *                readReceipts[], parentMessage }
 *   isMine   — bool
 *   onReply(message)
 *   onDelete(messageId)
 */
export default function MessageBubble({ message, isMine, onReply, onDelete }) {
  ...
  // FIX: authenticated download via fetch + Blob → no 401, no _blank tab
  const handleDownload = async (att) => {
    if (downloadingId === att.attachmentId) return; // prevent double-click
    setDownloadError('');
      ...
      <div className="msg-content">
        ...
        {message.attachments?.length > 0 && (
          <div className="msg-attachments">
            {downloadError && (
              <div style={{ color: 'var(--danger)', fontSize: 11, marginBottom: 4 }}>
                {downloadError}
              </div>
            )}
          ...
          <div className="msg-receipts">
            ...
            {message.readReceipts.length > 3 && (
              <span className="receipt-chip">
                +{message.readReceipts.length - 3} more
              </span>
            )}
          </div>
        )}
