...
export default function TaskItem({ task, myRole, onRefetch, allTasks = [] }) {
  ...
  const handleStatusChange = async (e) => {
    if (!canStatus) return;
    try { await taskApi.updateStatus(task.taskId, e.target.value); onRefetch(); } catch { /**/ }
  };

  const handleEditSave = async () => {
    if (!editName.trim()) return;
    setSaving(true);
    try { await taskApi.update(task.taskId, { name: editName }); onRefetch(); setEditing(false); } catch { /**/ }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!window.confirm(`Delete task "${task.name}"?`)) return;
    try { await taskApi.delete(task.taskId); onRefetch(); } catch { /**/ }
  };
  ...
  return (
    <div className="pm-task">
      <div className="pm-task-info">
        {editing ? (
          <div style={{ display:'flex', gap:6, marginBottom:4 }}>
            <input className="pm-field input" value={editName} onChange={e=>setEditName(e.target.value)}
          ...
          <div className="pm-task-name"
            style={{ textDecoration: task.status==='Done' ? 'line-through' : 'none', opacity: task.status==='Done' ? .5 : 1 }}>
            {task.name}
          </div>
        )}
        <div className="pm-task-meta">
          ...
          {task.dueDate && <span>Due {new Date(task.dueDate).toLocaleDateString([], {day:'numeric',month:'short'})}</span>}
      ...
      <div className="pm-task-actions" style={{ display:'flex', gap:6, alignItems:'center' }}>
        {canStatus && (
          <select value={task.status} onChange={handleStatusChange}
            style={{ background:'var(--charcoal)', border:'1px solid var(--divider)', borderRadius:'var(--radius)', color:'var(--light)', fontSize:11, padding:'3px 6px' }}>
            {STATUS_OPTIONS.map(s => <option key={s}>{s}</option>)}
          </select>
        )}
        {!canStatus && <StatusBadge status={task.status} />}
        {canEdit && (
          <>
