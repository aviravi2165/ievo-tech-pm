...
async function createAttachment({
  ...
  const { rows } = await pool.query(
    ...
    [uploadedByUserId, storedFileName, originalName, mimeType, fileSize, storagePath || storedFileName]
  );
  return rows[0];
}

/**
 * Links staged attachments to a message inside an open pg client (transaction).
...
/**
 * Returns attachment row if the user may access it
 * (uploader OR conversation participant).
 */
