...
// ── Activity members ──────────────────────────────────────────────────────────
router.get('/:id/members',         resolveActivityProject, requireRole('Member'),  ctrl.listMembers);
router.post('/:id/members',        resolveActivityProject, requireRole('Manager'), ctrl.addMember);
router.delete('/:id/members/:uid', resolveActivityProject, requireRole('Manager'), ctrl.removeMember);
...
router.get('/:activityId/tasks', resolveActivityProject, requireRole('Member'),
  async (req, res, next) => {
    try { res.json(await taskSvc.getTasksForActivity(req.params.activityId)); } catch(e){next(e);}
  }
);
router.post('/:activityId/tasks', resolveActivityProject, requireRole('Manager'),
  async (req, res, next) => {
